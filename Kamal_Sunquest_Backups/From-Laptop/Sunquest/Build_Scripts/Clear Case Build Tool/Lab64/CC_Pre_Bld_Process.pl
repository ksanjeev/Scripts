...
my $Email_Found;
my $Make_Baseline = "Y";
my $View_Name; # Added By Sharad Agarwal on 17th May 2007 for modifying the View name we are creating temporarily for Pre Build Process. 
...
#============================================================================#      
  ...
  if ((!defined $cfg{'MakeBaseline'}) or ($cfg{'MakeBaseline'} eq ""))
  {
    $Make_Baseline = "N";   
  }
  else
  {
    $Make_Baseline = $cfg{'MakeBaseline'};   
  }
...
foreach my $cfgrec (sort(@cfgsecs)) {
    ...
    # Added By Sharad Agarwal on 17th May 2007 for modifying the View name we are creating temporarily for Pre Build Process. 
    $View_Name = $project.'_Int_View';;
    ...
    #============================================================================#
    #   This will be used to create the label/Baseline Name
    #============================================================================#
    $build_no1 = '"'.$build_no.'"';
    ...
    #============================================================================#
    #   Before we proceed we need to delete the folder with the $View_Name from the storage location if exists.
    #$command_line = 'rmdir /S /Q '.$Storage_Location.$View_Name.'\.vws';
    
    # Added By Sharad Agarwal on 17th May 2007 for modifying the View name we are creating temporarily for Pre Build Process. 
    if (-d $Storage_Location.$View_Name.'.vws'){
        #system $command_line #  Delete the existing directory
        # Below 2 lines Commented by Ashwin on 22nd Nov 2006
        #$command_line = 'cleartool endview'.$View_Name;
        #result();
        
        #if ($result == 0) {
        #    print STDOUT "Command successfully executed : $command_line\n";
        #    print STDOUT "Deleted $Storage_Location $View_Name.vws\n";
        #}
        # Modified by Sharad Agarwal on 17th May 2007 for updating the View name used during Pre Build Process
        $command_line = 'cleartool rmview -force -tag '.$View_Name;
        result();        
    }
    ...
    # Modified by Sharad Agarwal on 17th May 2007 for updating the View name used during Pre Build Process
    $command_line = 'cleartool mkview -stream '.$int_stream.'@\\'.$product_vob.' -tag '.$View_Name.' '.$Storage_Location.$View_Name.'.vws';
    #   The below line has been commented by Ashwin for the above case.
    #$command_line = 'cleartool mkview -tag $View_Name -stream '.$int_stream.'@\\'.$product_vob.' -stgloc '.$Storage_Location;
    result();
    if ($result == 0) {
        print STDOUT "Command successfully executed : $command_line\n";
        print STDOUT "Info : Product[$product]: $View_Name created.\n";
    }
    ...
    # Added by Ashwin on 05/11/07
    #   Un mount all the current VOB's
    ...
    #============================================================================#
    #   Go to $View_Name\RAD_PVOB($product_vob) directory
    #   on M: drive
    #============================================================================#
    chdir ($View_Name);
    chdir ($product_vob);   
    ...
    #============================================================================#
    # Modified by Sharad Agarwal on 17th May 2007 for updating the View name used during Pre Build Process
    $command_line = 'cleartool mkbl -c '.$build_no1.' -view '.$View_Name.' '.$build_no;
    result();
    if ($result == 0) {
        print STDOUT "Command successfully executed : $command_line\n";
        print STDOUT "Info : Product[$product]: Baseline created for $View_Name view.\n";
    }
    ...
    if($Make_Baseline eq "Y")
    {
        ...
        if ($result == 0) {
            print STDOUT "Command successfully executed : $command_line\n";
            print STDOUT "Info : Product[$product]: $View_Name view ended.\n";
        }
        ...
        if ($result == 0) {
            print STDOUT "Command successfully executed : $command_line\n";
            print STDOUT "Info : Product[$product]: $View_Name view removed.\n";
        }
    ...
    else
    {
        ...
        # Source = M:\\.'$View_Name'
        print STDOUT "Source path : M:\\$View_Name\n";
        # Modified by Sharad Agarwal on 17th May 2007 for updating the View name used during Pre Build Process
        $command_line =  'xcopy /E /Y /I M:\\'.$View_Name.' '.$working_directory.$product_build_view;
    }
    #system $command_line;
