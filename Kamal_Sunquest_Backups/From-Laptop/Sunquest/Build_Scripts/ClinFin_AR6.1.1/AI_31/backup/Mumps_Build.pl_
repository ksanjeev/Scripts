...
sub get_config_data {
  ...
  $cfg{'prod'} = $cfgref_cc_build->val( $cfgsec_cc_build, 'Product' );
  ...
  $cfg{'blddir'} = $cfgref_cc_build->val( $cfgsec_cc_build, 'Working_Dir' ). $cfg{'prod'}.'_BLD_VIEW\\';
  die "Missing DB name in $cfgsec_cc_build of cc_build.cfg.\n"
...
#1 Added by Saju on 18th January, 2007 for the MUMPS folder
...
# end of 18th jan changes
  ...
  $cfg{'EmailAddress'} = $cfgref_cc_build->val( $cfgsec_cc_build, 'Email_List' );
...
# Commented and added by Saju on 18th January 2007, to use the key "MUMPS_Folder" from the MUMPS.cfg
...
if (!-d $bld_dir) {
  ...
  #3 Modified and added new by Saju on 18th January,2007
