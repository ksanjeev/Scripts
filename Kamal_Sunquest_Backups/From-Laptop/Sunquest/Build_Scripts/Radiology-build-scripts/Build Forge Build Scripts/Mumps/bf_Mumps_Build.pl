...
# 5) | 07th July 2008 | Kamalakannan     | Changes to package the class files as a new component
#					   Changes to package teh manager routine files as a new component		

# 6) | 01st October 2008 | Kamalakannan  | Changes to retain the first two lines of the file when packaging CDL
#					   		
# 7) | 22nd October 2008 | Kamalakannan  | Changed the script to remove the first two lines of the second file onwards when packaging routines with special requirements
#                                          Changed the script to remove the only one line of the second file onwards when packaging globals with special requirements
#
#=========================================================================
...
# Commented out by RZD on 07th April 2008
#my $cfgfile = "cc_build.cfg";
# Added by RZD on 7th April 2008
my $cfgfile = "mumps.cfg";
...
# Added by Kamalakannan Sanjeevan on July 07,2008
# Added the below subroutine to identify the folders to exclude for Manager routines packaging

sub get_mgr_excludes
{
  #my $rec = shift;
  my $ndx = 0;
  my @file_list = ();
  ...
  # Get the list of files to exclude if any
  if ((defined $cfg{'ExFileMgr'}) && ($cfg{'ExFileMgr'} ne "")) {
    $files = $cfg{'ExFileMgr'};
    # Added by Sharad Agarwal on 13th Oct 2006
    print STDOUT "exclude Manager files $files\n";
  }
  ...
  # Get the list of folders to exclude if any
  if ((defined $cfg{ExFoldMgr}) && ($cfg{ExFoldMgr} ne "")) {
    $files = $cfg{ExFoldMgr};
    # Added by Sharad Agarwal on 13th Oct 2006
    print STDOUT "exclude Manager folders $files\n";
    #print("We are here 2\n");
  }
...
# Added by Pavan Rayadurg on November 28,2008
# Added the below subroutine to identify the folders to exclude for CSQl routines packaging
...
# Added by Kamalakannan Sanjeevan on 7th July 2008
# Added the below if-elsif condition to check for Special requirementa
# Package the code based on the special requirement -  This is to check for AI Templates
# Modified by Kamalakannan Sanjeevan on 22nd October 2008
# Added elsif condition to differentiate globals or routines packaging separately

if ($cfg{'SplReq'} =~ /Y/i && uc($file_ext) =~ /GSA$/i )
{
    if ($first_out == 0)
    {
       shift @src_lines;
...
elsif ($cfg{'SplReq'} =~ /Y/i && uc($file_ext) =~ /RSA$/i )
{
    if ($first_out == 0)
    {
       shift @src_lines;
...
# Added by Kamalakannan Sanjeevan on 1st October 2008
# Added the below if-elsif condition to check for Special requirements and CDL extensions
# Checks if Special Requirements is not AI_Templates and files with CDL extension
# Not to remove the first two lines of the CLD files while packaging
elsif ($cfg{'SplReq'} !~ /Y/i && uc($file_ext) =~ "CDL")
{
     if ($first_out == 0)
    {
       #shift @src_lines;
...
elsif ($cfg{'SplReq'} !~ /Y/i && uc($file_ext) !~ "CDL")
{
...
} #End Add on 7th July 2008
  
  # Commented by Saju on 7th Dec 2006
  # open OUT, ">>$out_file" or die "\nUnable to open: $out_file\nError: n$!\n";
  
  my $line = '' if ($cfg{'StripHeader'} =~ /n/i);
  my $flag = 'none' if ($cfg{'StripHeader'} =~ /n/i);
  ...
  # Modified by Kamalakannan Sanjeevan on 7th July 2008
  # Check for Special requiremet is added to process for AI Templates
    
    if ($cfg{'StripHeader'} =~ /n/i && $cfg{'SplReq'} !~ /Y/i)
    {
       # Process for the AI builds
        if (defined $flag)
        {
 	 ...
 	 # Check for Special requiremet is added to process for AI Templates
    elsif ($cfg{'StripHeader'} =~ /n/i && $cfg{'SplReq'} =~ /Y/i)
    {   # Process for the COMLAB,LAB builds
        if (defined $flag)
        {
 		 ...
 		 # Added the below elsif condition to add an empty line at the beginning of the packaging
             elsif (($_ =~ /^(\s)*\n$/))
            {
                $line = '';
                $flag = 'none';
	...
	# Added by Kamalakannan Sanjeevan on 7th July 2008
 	 # Added the below if condition to identify the occurrence of ***DONE*** and set the flag to none
            if ($flag eq 'none') 
            {
                if ($line =~ m/\*\*\*DONE\*\*\*/)
                {
    ...
    # Added  by kamalakannan on 3rd Oct 2008
    # Added condition to check for Condition if not CDL Files
    elsif ($cfg{'StripHeader'} =~ /n/i && uc($file_ext) !~ "CDL")
    {   # Process for builds other than COMLAB like for RAD,AI,IX
        next if ($_ =~ /^(\s)*\n$/);
        next if ($_ =~ /^(?:\s|\.)*;(?:[^;]|$)/);
        print OUT "$_";
    ...
    # Added  by kamalakannan on 3rd Oct 2008
    # Added the below condition to check for Condition if CDL Files
    # It does not remove the blank spaces when packaging the files
    elsif ($cfg{'StripHeader'} =~ /n/i && uc($file_ext) =~ "CDL")
    {   # Process for builds other than COMLAB like for RAD,AI,IX
        #next if ($_ =~ m/ /);
        #print OUT "$_";
...
sub do_build
{
  ...
  # Process the mumps files under the current directory.
    ...
    # Added by Kamalakannan Sanjeevan on 22nd October 2008
    # Added the if condition to append a line if it is a global and with special requirements
    if (($out_file =~ /.go$/i) && ($cfg{'SplReq'} =~ /Y/i))
    {
        #print OUT "\n";
        print OUT "***DONE***";
    }
...
sub get_config_data {
  ...
  $cfg{'prod'} = $cfgref->val( 'MUMPS', 'Product' );
  die "Missing product name in Mumps of mumps.cfg.\n"
    if (!defined $cfg{'prod'});
  ...
  #die "Missing DB name in $cfgsec of $cfgfile.\n"
   #Commented out by RZD on 7th April 2008
  #$cfg{'blddir'} = $cfgref->val( 'MUMPS', 'Working_Dir' ). $cfg{'prod'}.'_BLD_VIEW\\';
  #die "Missing DB name in Mumps of mumps.cfg.\n"
  #Added out by RZD on 7th April 2008
  $cfg{'blddir'} = $ARGV[1]; # Should be fed thru Command Line...
  die "Missing DB name in Mumps of mumps.cfg.\n"
...
# End of 02nd Mar 2007 changes
# Added by RZD on 7th April 2008
$cfg{'Build_No'} = $ARGV[0]; # Should be fed thru Command Line.
...
# Added by Kamalakannan Sanjeevan on 7th July 2008
# Added the below two config definitions to identify the class files and manager routines from the configuration file

  $cfg{'ClsFiles'} = $cfgref->val( $cfgsec, 'ClassFiles' );
  die "Missing Class File name in $cfgsec of $cfgfile.\n"
    if (!defined $cfg{'ClsFiles'});
   
    $cfg{'MgrFiles'} = $cfgref->val( $cfgsec, 'ManagerRoutines' );
  die "Missing Manager Files in $cfgsec of $cfgfile.\n"
    if (!defined $cfg{'MgrFiles'});
...
# Added by Kamalakannan Sanjeevan on 7th July 2008
# Added the below config definitions to identify the class output file from the configuration file

  $cfg{'ClsOut'} = $cfgref->val( $cfgsec, 'ClassOutput' );
  die "Missing Class Output in $cfgsec of $cfgfile.\n"
    if (!defined $cfg{'ClsOut'});
...
# Added by Kamalakannan Sanjeevan on 7th July 2008
# Added the below three config definitions to identify the manager output file and Files and folders to exclude for manager routines from the configuration file

 $cfg{'MgrRtnOut'} = $cfgref->val( $cfgsec, 'ManagerRoutineOutput' );
  die "Missing Global Output in $cfgsec of $cfgfile.\n"
    if (!defined $cfg{'MsqlRtnOut'});

 $cfg{'ExFileMgr'} = $cfgref->val( $cfgsec, 'ExcludeMgrFile' );
  die "Missing excluded Manager files in $cfgsec of $cfgfile.\n"
    if (!defined $cfg{'ExFileMgr'});
       
  $cfg{'ExFoldMgr'} = $cfgref->val( $cfgsec, 'ExcludeMgrFolderList' );
  die "Missing Manager excluded folders in $cfgsec of $cfgfile.\n"
    if (!defined $cfg{'ExFoldMgr'});
...
# End add on 17th July 2007
  ...
  # Added by Kamalakannan Sanjeevan on 7th July 2008
  # Added the below config definitions to identify the special requirements based on the configuration file

  $cfg{'SplReq'} = $cfgref->val( $cfgsec, 'SpecialRequirement' );
  die "Missing Special requirement in $cfgsec of $cfgfile.\n"
    if (!defined $cfg{'SplReq'});
...
foreach $component (@bld_components)
{
    ...
    if ($component eq "RTN") {
        ...
        # Get our list of excluded files and sub-directories if any
        get_excludes();
     }	elsif ($component eq "CLD"){
	# Added by Kamalakannan Sanjeevan on 7th July 2008
	# Added the following elsif condition to create the Class output file and package them
    
        #5 Modified and added new by Saju Carlos on 02nd Mar 2007 for Mumps filename changes
        # $bld_name = $bld_dir . "\\" . $cfg{'prod'} . "_" . get_date() . $cfg{'GblOut'};
        $bld_name = $bld_dir . "\\" . $cfg{'prod'} . "_" . $cfg{'Build_No'} . "_" . get_date() . $cfg{'ClsOut'};
        # End of 02nd Mar 2007 changes
        
        
        # get our file pattern hash for each type of component
        get_patterns($cfg{'ClsFiles'});
        # Get our list of excluded files and sub-directories if any
        get_excludes();
       
    }  elsif ($component eq "MRTN"){
  	# Added by Kamalakannan Sanjeevan on 7th July 2008
	# Added the following elsif condition to create the Manager output file and package them
        ...
        $bld_name = $bld_dir . "\\" . $cfg{'BldSepFold'} . "_" . $cfg{'Build_No'} . "_" . get_date() . $cfg{'MgrRtnOut'};
        # End of 02nd Mar 2007 changes
    ...
    elsif ($component eq "MSQLGBL")
    {
       if ($cfg{'prod'} ne "RAD")
       {
            ...
            #$bld_name = $bld_dir . "\\" . $cfg{'BldSepFold'} . "_" . $cfg{'Build_No'} . "_" . get_date() . $cfg{'MsqlOut'};
