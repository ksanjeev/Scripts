...
#added version rzd
my ($prod, $buildDir, $log_file, $log_error,$myVersion);
my $command;
my $file_del;
# Commented by Sharad Agarwal on 09th March 2007
#my $Email_flag = 1;
...
#============================================================================#
  for $i (0..(scalar(@data)-1))
  {
    ...
    #********************************************************************************
    if ( uc($command) eq "UN-NETREGISTER")
    {
    # Comment added by Robert Daniels on 16th July 2007
    # If the first parameter is UN-NETREGISTER in the input file (GUI.cfg)
    # It is used for registering the DLL .NET COMONENTS. 
    # The config file should contain in the format;
    # Ex: Register,C:\Level1\,abc.dll
    #     Register,C:\Level1\,abc.exe
    #********************************************************************************
     ...
     #$command_exec = 'C:\WINNT\Microsoft.NET\Framework\v1.1.4322\RegAsm /UnRegister "'. $data[$i][1]. $data[$i][2].'"';
     #Modified by Sharad Agarwal on 18th July 2007.
     #The path needs to be set in the Enviroment variables.
     $command_exec = 'RegAsm /UnRegister "'. $data[$i][1]. $data[$i][2].'"';
      
      $log_details = "Error : UN-Registering ". $data[$i][1]. $data[$i][2]. "\n";
      result();
    }
    #********************************************************************************
    #********************************************************************************
    if ( uc($command) eq "NETREGISTER")
    {
    # Comment added by Robert Daniels on 16th July 2007
    # If the first parameter is REGISTER in the input file (GUI.cfg)
    # It is used for registering the DLL .NET COMONENTS. 
    ...
    # Ex: Register,C:\Level1\,abc.dll
    #     Register,C:\Level1\,abc.exe
    #condition added by Robert Daniels on 22 July 2008 CR:1-7378WT
    #added codebase switch to regasm command
    #********************************************************************************
      my @line_data = split /\./, $data[$i][2];
      
     #$command_exec = 'C:\WINNT\Microsoft.NET\Framework\v1.1.4322\RegAsm "'. $data[$i][1]. $data[$i][2].'"';
     #Modified by Sharad Agarwal on 18th July 2007.
     #The path needs to be set in the Enviroment variables.
      $command_exec = 'RegAsm "'. $data[$i][1]. $data[$i][2].'"' . ' ' . '/codebase';
    ...
    #********************************************************************************
        ...
        #********************************************************************************
    if ( uc($command) eq "REGTLIB")
    {
    # Comment added by Robert Daniels on 16th July 2007
    # If the first parameter is REGISTER in the input file (GUI.cfg)
    # It is used for registering the Type Libraries for .NET COMONENTS. 
    # The config file should contain in the format;
    # Ex: Register,C:\Level1\,abc.dll
    #     Register,C:\Level1\,abc.exe
    #********************************************************************************
     ...
     $command_exec = 'RegTLib "'. $data[$i][1]. $data[$i][2].'"';
    ...
    # The config file should contain in the format;
    # Ex: javac,C:\j2sdk1.4.2_05\lib\java40.jar,C:\RAD_BLD_VIEW\RAD_GUI\WebView\java\SQInputField\,SQInputField.java
    #********************************************************************************
    ...
    #********************************************************************************
    if( uc($command) eq "VB6"){
    #condition added by Robert Daniels on 30 April 2007
    # Calls subroutine that updates vbp project file version to current build version
        my $myFileName = $data[$i][1] . $data[$i][2];
        #my $myFileName = $data[$1][1]
        #print "$myFileName\n";
        updateVbVersion($myFileName);
    }
    #********************************************************************************
    if (uc($command) eq "NETCONFIG"){
    #Condition added my Robert Daniels on 02 May 2007
    #copies master.config file to path and renames file
        my $myPath = $data[$i][1];
        my $myFileName = $data[$i][2];
        my $configDir = qq("C:\\temp\\master.config");
        $myPath = qq("$myPath");
        $log_details = "Error : not able to copy master.config file to $myPath\n";
        $command_exec = 'copy /Y ' . $configDir ." " . $myPath;
        #print "$command_exec\n";
        result();
        $myPath = $data[$i][1];
        $myFileName = $data[$i][1] . $myFileName;
        $myPath = $myPath . "master.config";
        $myPath = qq("$myPath");
        $myFileName = qq("$myFileName");
        $log_details = "Error : not able to rename master config file to $myFileName\n";
        #print "command2\n";
        $command_exec = 'move /Y ' . $myPath . " " . $myFileName;
        #print "$command_exec\n";
        result();
    }
    #********************************************************************************
    if (uc($command) eq "NET"){
    #condition added by Robert Daniels on 17 May 2007
    #Calls subroutine that updates vbp project file version to current build version
        print "In Subroutine NET\n";
        my $myFileName = $data[$i][1] . $data[$i][2];
        updateAssemblyVersion($myFileName);
    }
...
#============================================================================#
sub updateAssemblyVersion{
# Added by Robert Daniels on 17 May 2007
# This function updates a AssemblyInfo.vb file to the current version
    my $myAssemblyFile = $_[0];
    my $testValue = '<Assembly: AssemblyVersion';
    my $testValue2 = '*';
    
    open OUT, "<$myAssemblyFile" or die "\nUnable to open $myAssemblyFile\nError: $!\n";
    #@data holds all the elements of current AssemblyInfo.vb project file. 
    my @data = <OUT>;
    close(OUT) or die "Unable to close: $myAssemblyFile\nError: $!\n";;
    for (my $i=0;$i<@data;$i++){

        #if(index($data[$i],$testValue) != -1){
        if((index($data[$i],$testValue) != -1) &&(index($data[$i],$testValue2) == -1)){
            #print $data[$i];
            #print "\n";
            $data[$i]='<Assembly: AssemblyVersion("' . $myVersion .'")>';
            #print "new value\n";
            #print $data[$i];
            #print "\n";
        }
...
#============================================================================#
sub updateVbVersion{
# Added by Robert Daniels on 30 April 2007
# This function updates VBP project file to the current version
    #print "Running updateVbVersion subroutine\n";
    
    #if(-e "cc_build.cfg"){
     #   print "Found file\n";
  #  }
    my $myVBProjectFile = $_[0];
    my $testValue="MajorVer";
    #my $mycfgfile = "cc_build.cfg";
    #my $mycfg = Config::IniFiles->new(-file => "cc_build.cfg" );
    #my $myVersion = $mycfg->val( 'CLEARCASE', 'Build_No' ); 
    my @myValues = split('\.', $myVersion);

    my $myMajorVer = $myValues[0];
    my $myMinorVer = $myValues[1];
    my $myRevisionVer = $myValues[3];
    #my $myRevisionVer = $myValues[2];
   # print "$myVBProjectFile\n";
    # Open a vbs file file for reading and store the data into an array.
    open OUT, "<$myVBProjectFile" or die "\nUnable to open $myVBProjectFile\nError: $!\n";
    my @data = <OUT>;
    close(OUT) or die "Unable to close: $myVBProjectFile\nError: $!\n";
    #@data holds all the elements of current vbs project file. 
    
    
    
    for (my $i=0;$i<@data;$i++){
        if(index($data[$i],$testValue) != -1){
            $data[$i] = "MajorVer=". $myMajorVer . "\n";
            $i +=1;
            $data[$i] = "MinorVer=" . $myMinorVer  . "\n";
            $i +=1;
            $data[$i] = "RevisionVer=" . $myRevisionVer . "\n";
        }
    }
    open OUT, ">$myVBProjectFile" or die "\nUnable to open $myVBProjectFile\nError: $!\n";
    print OUT @data;
...
#============================================================================#      
  ...
  $myVersion = $cfgref_cc_build->val( 'CLEARCASE', 'Build_No' );
