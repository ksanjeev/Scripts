...
#  the necessary data from the cc_build.cfg file
#
# Usage:
#  
#
...
# 4) | 13th Oct 2006  | Sharad Agarwal   |
# 5) | 16th Oct. 2006 | Sharad Agarwal   |
# 6) | 14th Nov. 2006 | Sharad Agarwal   |
# 7) | 1st Dec. 2006  | Sharad Agarwal   |
# 8) | 07th Dec. 2006 | Sharad Agarwal   |
# 9) | 7th Dec 2006   | Saju Carlos      | 1) To remove the problem of extra '0D' comming in the string "0D 0A 0D 0D 0A" in output file.
#                                          2) To add three lines at the end of the file.
# 10) | 18th Jan 2007 | Saju Carlos      | Added a new key 'MUMPS_FOLDER' into the configuration file for mumps folder
# 11) | 13th Feb 2007 | Sharad Agarwal   |
# 12) | 28th Feb 2007 | Saju Carlos      | Change of name format for Mumps ro files from product_date_time.ro to product_date.ro
# 13) | 02nd Mar 2007 | Saju Carlos      | Change mumps file name from the current form product_date.ro to product_version_date.ro
...
sub add_file {
  ...
  close SRC or die "\nUnable to close: $file_name\nError: $!\n";
    ...
    if (uc($file_ext) =~ "RSA") {
    if ((($src_lines[0] !~ /^Cache/) && ($src_lines[0] !~ /^Open M/)) ||
       ($src_lines[1] !~ /^\%RO /)) {
    ...
    if ((($src_lines[0] !~ /^\~Format/) && ($src_lines[0] !~ /^\n/) &&
       ($src_lines[0] !~ /^Cache/) && ($src_lines[0] !~ /^Open M/)) ||
       ($src_lines[2] !~ /^\^(\D)+/)) {
      print STDERR "$file_name has a bad or missing header.\n";
      # Added by Sharad Agarwal on 13th Oct 2006
      print STDOUT "$file_name has a bad or missing header.\n";
...
sub get_config_data {
  ...
  #Edited by Robert Daniels on 2 May 2007.
  #$cfg{'blddir'} = $cfgref->val( 'CLEARCASE', 'Working_Dir' ). $cfg{'prod'}.'_BLD_VIEW\\';
  $cfg{'blddir'} = $cfgref->val( 'CLEARCASE', 'BuildDirectory');
  #print "$cfg{'blddir'}\n";
...
# Added by Saju on 18th Jan 2007 for the MUMPS folder
...
# Modified by Sharad Agarwal on 09th March 2007
#($Version) =  ($cfg{'Build_No'} =~ /(.+)(?=\.\d+)/);
$Version = $cfg{'Build_No'};
# End of 02nd Mar 2007 changes
...
foreach $component (@bld_components) {
    ...
    if ($component eq "RTN") {
        ...
        $bld_name = $bld_dir . "\\" . $cfg{'prod'} . "_" . $Version . "_" . get_date() . $cfg{'RtnOut'};
        # End of 02nd Mar 2007 changes
        ...
        $bld_name = $bld_dir . "\\" . $cfg{'prod'} . "_" . $Version . "_" . get_date() . $cfg{'GblOut'};
        # End of 02nd Mar 2007 changes
        ...
        $bld_name = $bld_dir . "\\" . $cfg{'BldSepFold'} . "_" . $Version . "_" . get_date() . $cfg{'MsqlRtnOut'};
        # End of 02nd Mar 2007 changes
       ...
       if ($cfg{'prod'} ne "RAD") {
              ...
              $bld_name = $bld_dir . "\\" . $cfg{'BldSepFold'} . "_" . $Version . "_" . get_date() . $cfg{'MsqlOut'};
              # End of 02nd Mar 2007 changes
