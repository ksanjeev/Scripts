...
# Modified by Kamalakannan Sanjeevan on 30th Jan 2008
# Modified the next 2 lines to avoid help for -undo and -findmerge options

$SYNOPSIS = "$NAME [-help] [-ignore] [-graphical] 
               {-redo | -print } [activity]";

# Removed by Kamalakannan Sanjeevan on 30th Jan 2008
# Removed the arguments and description for -undo and -findmerge options
$ARGUMENTS = "
...
";
# Modified by Kamalakannan Sanjeevan on 30th Jan 2008
# Modified to skip the return status values -undo and -findmerge options
...
# Removed by Kamalakannan Sanjeevan on 30th Jan 2008
# Removed the next 5 lines to disable the -undo and -findmerge option 
...
# Removed by Kamalakannan Sanjeevan on 30th Jan 2008
# Removed the next 3 lines to disable the -undo option 
...
# Removed by Kamalakannan Sanjeevan on 30th Jan 2008
# Removed the next if tag to disable the -undo option 
#if ( $opt_undo ) {
   #------------------------------------------------
...
#------------------------------------------------
# Removed by Kamalakannan Sanjeevan on 30th Jan 2008
# Removed the next 3 lines to disable reversing used by -undo option 
...
foreach ( @cset ) {
   next if ( ! "$_" );
   # Removed by Kamalakannan Sanjeevan on 30th Jan 2008
   # Removed the next 3 lines to skip printing version based on the option 
...
# print ("\ncmd = $cmd\n");
   ...
   #-------------------------------------
   # Removed by Kamalakannan Sanjeevan on 30th Jan 2008
   # Removed the next 7 lines to skip performing merge based on -undo and -findmerge option 
...
sub prompt_for_stream {
   ...
   foreach $stream ( @streams ) {
      # Modified by Kamalakannan Sanjeevan on 30th Jan 2008
      # Modified the next line to skip checking for -findmerge option 
