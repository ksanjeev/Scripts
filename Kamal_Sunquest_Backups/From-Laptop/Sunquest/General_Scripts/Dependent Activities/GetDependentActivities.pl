...
#
#=========================================================================
# GetDependentActivities.PL
#      This script fetches the versions of an element associated with a activity from Clearcase.
#      The script gets input from the configuration file ( GetDependentActivities.CFG ). 
#
#
# Usage:
#       1) Edit the configuration file ( GetDependentActivities.CFG )
#       2) Run this script at command prompt as 'perl GetDependentActivities.PL' 
#
#
# Returns:
#      1) It creates folder for each activity specified in the "GetDependentActivities.CFG" file and copies the associated versions of elements as per the change set.
...
# Edit the below variables as per required
    ...
    my $ConfigFile = 'GetDependentActivities.CFG';            # Absolute path to the "GetDependentActivities.CFG"
    my $ViewTag = 'LAB_6.3.0_MHI1_int_view';
...
#-------------------- Please do not edit below this line -----------------#
  ...
  ##################### Start of Main Program #####################
    ...
    #$Build{DESTINATION_FOLDER} .= 'Obj-Extract\\';
    my $ErrorLog = $Build{DESTINATION_FOLDER}.'Error_GetDependentActivities.log';  
    my $EventLog = $Build{DESTINATION_FOLDER}.'GetDependentActivities.log';

    print "\n\n* * * Starting GetDependentActivities.PL.......* * * \n\n";
    ...
    # Go create a baseline out of the delivered code
    #$command_line = 'cleartool mkbl -c '.$Build{build_no}.' -view '.$ViewTag.' '.$Build{build_no};
    
       
    # Get the LIST of activities
    #my @activities = split(/,/,$Build{ACTIVITIES});
    my @activities = sort(split(/,/,$Build{ACTIVITIES}));
    ...
    # check for the undelivered activities
    &deliveract();
    ...
    foreach (@activities)
    {
        ...
        my $activitydir = $Build{DESTINATION_FOLDER};
    ...
    print "\n\n* * * End of GetDependentActivities.PL.......* * * \n";
...
#************** Start of deliveract() **********************
  ...
  sub deliveract
  {
    ...
    my $command =' ';
    $command = 'cleartool deliver -preview -s -stream '.$Build{TARGET}.'@\\'.$Build{PROJECT_VOB};
    
    my @deliverInfo = `$command`;
    
    # Check the exit status
    if (@deliverInfo)
    {
      if ($deliverInfo[0] =~ /^Changes will be POSTED/)
      {
        map { print LOG } @deliverInfo;
        print LOG "\n";
      }
      elsif ($deliverInfo[0] =~ /^cleartool: Error: Deliver operation already/)
      {
        printlog("\nUnable to generate the list of activities when delivery is in progress. \n", 'error');
        printlog("\n ----- End event log ------ \n","");
        printlog("\n ----- End error log ------ \n","error");
        print("\n ----- Please check error log ------ \n");
        exit;
...
=cut
  ...
  #************** Start of GetChangeSet() **********************
    ...
    sub GetChangeSet
    {
        ...
        @ChangeSet = sort(grep {/^\s*M\:(?:.+)\\\d+$/} @activityInfo);
          ...
          if (/(?:(.+)(?=\@\@))/)
          {
            ...
            if (defined $cshash->{$key})
            {
               if ($cshash->{$1} =~ /(?:.+)\\(\d+)/)
               {
                ...
                my ($new_version) = (/(?:.+)\\(\d+)/);
                my $tempversion = $current_version + 1; 
                if ($new_version ne $tempversion)
                {
                    printlog("Activity has a Dependant version");
                }
                else
                {
