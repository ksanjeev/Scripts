...
sub add_file {
  ...
  my $last_line = '' if ($cfg{'StripHeader'} =~ /y/i);
  foreach (@src_lines)
  {
    if ($cfg{'StripHeader'} =~ /y/i)
    {   # Process for the COMLAB build
        if ($_ =~ /^(\s)*\n$/)
        {
            print OUT "$_" if ($last_line =~ /^(?:\^(?:.+)\((?:.+)\))/i);
            $last_line = '';
            next
        }
        # next if (($_ =~ /^(?:\s|\.)*;(?:[^;]|$)/)&&($file_ext =~ /GSA/i));
        $last_line = $_ if ($file_ext =~ /GSA/i);
        print OUT "$_";
    }
    elsif ($cfg{'StripHeader'} =~ /n/i)
    {   # Process for builds other than COMLAB like for RAD,AI,IX
  ...
  # print OUT "\x0d\x0a";
    ...
    {   # Process for the COMLAB build
        ...
        elsif ($file_ext =~ /GSA/i)
        { # All the GSA's in COMLAB should not have new lines added
            # print OUT "\x0a" if ($last_line =~ /^(?:\^(?:.+)\((?:.+)\))/i);
        }
    }  
    elsif ($cfg{'StripHeader'} =~ /n/i)
    {   # Process for builds other than COMLAB like for RAD,AI,IX
