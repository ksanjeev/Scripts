...
print "\tTHE OUPUT OF THE CACHE VERSION AND DATABASE TYPE USING METHOD 1\n";
print "\t---------------------------------------------------------------\n";
...
foreach my $i (@patch)
{
print "The contents of Patches array is$i\n";
}
...
# To capture the tapes names in a file
system("sh tape.sh");
...
foreach my $i (@tapeDrive)
{
  $i =~ s/\s+/:/g;
  $i =~ s/:/ /g;
  print "The Tape Drives found in the server = $i\n";
}
print "\n"; 
